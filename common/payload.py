...
def decode_payload(raw: bytes) -> dict:
    ...
    if n == ENV_LEN:
        ...
        version, msg_type, node_id, seq, temp_x100, hum_x100, batt_mv, flags = struct.unpack("<BBBBhHHB", base)
        if msg_type != MSG_ENV:
    ...
    raise ValueError(f"Bad payload length: {n} (expected {ENV_LEN} or {CAM_LEN})")